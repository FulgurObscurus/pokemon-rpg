...
function safeLocalStorageSet(key, value) {
  try {
    ...
    // Частая причина проблем: QuotaExceededError после попыток кэшировать покедекс
    try { localStorage.removeItem('pokemonData151'); } catch(_) {}
    try {
      localStorage.setItem(key, value);
      return true;
    } catch (e2) {
      console.error('localStorage setItem failed:', e2);
      return false;
    }
  }
}
...
function saveGame() {
  try {
    ...
    const state = {
      ...
      items: {
        potion: Math.max(0, parseInt(items.potion || 0, 10) || 0),
        pokeball: Math.max(0, parseInt(items.pokeball || 0, 10) || 0),
      },
      log: Array.isArray(gameLog) ? gameLog.slice(-50) : [],
      party: (myParty || []).map(p => ({
        speciesId: p.speciesId,
        level: (typeof p.level === 'number') ? p.level : (parseInt(p.level, 10) || 1),
        exp: (typeof p.exp === 'number') ? p.exp : (parseInt(p.exp, 10) || 0),
        currentHp: (typeof p.currentHp === 'number') ? p.currentHp : (parseInt(p.currentHp, 10) || 0),
        status: p.status || null,

        // стабильные поля (иначе после F5 “переролл”)
        ability: p.ability || null,
        shiny: !!p.shiny,
        gender: p.gender || null,
        heldItem: (typeof p.heldItem !== 'undefined') ? p.heldItem : null,
        friendship: (typeof p.friendship === 'number') ? p.friendship : (parseInt(p.friendship, 10) || 0),

        moves: (p.moves || []).map(m => ({
          id: (m && m.id) ? m.id : null,
          name: (m && m.name) ? m.name : null,
          pp: (m && typeof m.pp === 'number') ? m.pp : (m ? parseInt(m.pp, 10) : null),
        })),
    ...
    const ok = safeLocalStorageSet('pokemonRPG_save_v3', JSON.stringify(state));
    if (!ok) console.warn('Сохранение не записалось (возможно, переполнен localStorage)');
    return ok;
  } catch (e) {
...
function loadGame() {
  try {
    ...
    try {
      ...
      // реально битый JSON — лучше убрать, чтобы не ломал запуск каждый раз
    ...
    // НЕ удаляем сейв из-за версии — просто пытаемся загрузить
    if (state.version < SAVE_VERSION) {
      console.warn('Старое сохранение: версия ' + state.version + ', текущая ' + SAVE_VERSION);
    }

    // если данных покемонов ещё нет — просто подождём следующий запуск (не трогаем сейв)
