...
function buildMoveFromEntry(moveEntry) {
    const moveId = moveEntry.id || moveEntry.moveId || moveEntry.key || moveEntry.slug || moveEntry.en || moveEntry.name;
...
class Poke {
    constructor(speciesId, level = 5) {
        ...
        this.moves = (base.moves || []).slice(0, 4).map(buildMoveFromEntry);
            ...
            if (m.learnLevel === this.level) {
                ...
                if (!this.moves.some(mm => mm.id === move.id || mm.name === move.name)) {
                    if (this.moves.length < 4) {
                        ...
                        addMessage(`⚠️ У ${this.name} уже 4 приёма, замените вручную.`);
                    }
                }
            }
        }
